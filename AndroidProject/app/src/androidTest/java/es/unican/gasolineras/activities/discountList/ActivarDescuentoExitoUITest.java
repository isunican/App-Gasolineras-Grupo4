...
@HiltAndroidTest
public class ActivarDescuentoExitoUITest {
    @Rule(order = 0)
    ...
    public void desactivarDescuentoExito() {
        ...
        descuento2.onChildView(withId(R.id.chkActive)).check(matches(isNotChecked()));

        descuento2.onChildView(withId(R.id.chkActive)).perform(click());
        descuento2.onChildView(withId(R.id.chkActive)).check(matches(isChecked()));
    }
}
